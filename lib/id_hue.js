...
module.exports = function(socket) {
	...
	socket.addListener(function(ioclient) {
		ioclient.on('hue', function(data) {
			...
			if (data.action === 'discovery') {
				...
				_on(data.bridge, data.light.id, function(err) {
					ioclient.emit('hue-bridge-lights', {err: err});
				});
			} else if (data.action === 'off') {
				_off(data.bridge, data.light.id, function(err) {
					ioclient.emit('hue-bridge-lights', {err: err});
				});
			}
		});
	...
	function _on(bridge, lightId, callback) {
		var state = lightState.create();
		var hueApi = new hue.HueApi(bridge.ipaddress, bridge.user.user_id);
		hueApi.setLightState(lightId, state.on(), function(err, result) {
			if (typeof callback !== 'undefined')
				callback(err, result);
			else
				console.log('Error setting light state to on for light id ' + lightId);
		});
	}

	function _off(bridge, lightId, callback) {
		var state = lightState.create();
		var hueApi = new hue.HueApi(bridge.ipaddress, bridge.user.user_id);
		hueApi.setLightState(lightId, state.off(), function(err, result) {
			if (typeof callback !== 'undefined')
				callback(err, result);
			else
				console.log('Error setting light state to on for light id ' + lightId);
	...
	return {
		turnOn: function(bridge, lightId, callback) {
			_on(bridge, lightId, callback);	
		},

		turnOff: function(bridge, lightId, callback) {
			_off(bridge, lightId, callback);
		}
	}
