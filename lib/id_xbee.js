...
module.exports = function(lcd, pushover, socket, wemo, hue) {
  ...
  function onDeviceExit(id, beaconAddr) {
    Bleep.findById(id)
  ...
  function onDeviceEnter(id, beaconAddr) {
    Bleep.findById(id)
      .populate('beacons') // only return the Persons name
      .exec(function (err, bleep) {
  ...
  function saveUpdatedBleep(bleep, exits, enters) {
    ...
    bleep.save(function (err, bleep, numberAffected) {
      ...
      enters.forEach(function(beacon) {
        onDeviceEnter(bleep._id, beacon.address);
      });

      exits.forEach(function(beacon) {
        onDeviceExit(bleep._id, beacon.address);
      });
    });
  }
